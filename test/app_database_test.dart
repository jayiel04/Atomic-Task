...
void main() {
  ...
  test(
    'migrates version 9 by adding the nullable reminder sound column',
    () async {
      final sqliteDatabase = sqlite3.openInMemory();
      sqliteDatabase.execute('''
        CREATE TABLE task_recurrence_rules (
          ...
          start_date INTEGER NOT NULL,
          end_date INTEGER NULL,
          reminder_time_minutes INTEGER NULL,
          is_active INTEGER NOT NULL DEFAULT 1,
          created_at INTEGER NOT NULL,
          updated_at INTEGER NOT NULL
        );
        ...
        INSERT INTO tasks
          (title, is_completed, created_at, updated_at)
        VALUES ('Tarea previa al sonido propio', 0, 1786348800000, 1786348800000);
      ...
      // La tarea existente se conserva y el sonido propio queda en NULL
      // («Predeterminada»).
      var task = (await dataSource.watchAll().first).single;
      expect(task.title, 'Tarea previa al sonido propio');
      expect(task.reminderSoundKey, isNull);

      // El sonido propio se persiste y se limpia junto con la alarma.
      final reminderAt = DateTime(2026, 8, 10, 10);
      await dataSource.updateWithReminder(
        id: task.id,
        title: task.title,
      ...
      task = (await dataSource.watchAll().first).single;
      expect(task.reminderAt, reminderAt);
      expect(task.reminderSoundKey, 'pulso_electronico');
